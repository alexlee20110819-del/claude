 ...
 *
 * Two quirks of the real Queensland workbook drive this file:
 *
 ...
 *     *hyperlink*, not its value. Hyperlink targets are extracted into a
 *     row/column map before parsing, and the cell text is only used when it is
 *     itself a URL.
 *
 * Everything here treats file content as inert data: no formula evaluation
 * (`cellFormula: false`), no HTML, and URLs are validated before use.
 *
 * SheetJS is loaded on demand — it is by far the heaviest dependency, and the
 * dashboard itself never needs it after the initial import.
 */

import type { ImportResult, ImportedLead, LeadDataset } from '@/types/lead';
import {
  decodeXmlEntities,
  makeLeadKey,
  safeUrl,
...
const PREFERRED_SHEET = 'All Current Leads';

/** Hyperlink targets by row index, then column index. */
type HyperlinkMap = Map<number, Map<number, string>>;
...
export async function importLeadFile(file: File): Promise<ImportResult> {
  ...
  return buildDataset({ rows, hyperlinks, sheetName, fileName: file.name });
}
